...
- **FW ABC は過去月も引ける**（`--month 2025-12` のように任意の月を渡せる）。したがって
  ...
  - ⚠ かつてここには「過去月の商品明細は返らない」と書いてあったが**誤りだった**。
    一度の失敗を一般化したもので、検証すると普通に返る。同じ誤りを 月別日別売上推移でも
    やっている（「対象月を受け付けない」→実際は判定コードのバグ）。
    **「FWにデータが無い」と結論づける前に、必ず取れた数字を独立した値で検算すること。**
...
- 遡り取り込み: `month` は複数・範囲で渡せる（`2025-11,2025-12` / `2024-09..2026-07`）。
  1店1ログインで月を回すので、23ヶ月でもログインは1回。1ヶ月あたり約70秒。
...
- FWで取りきれない粒度は、ダイニー管理アプリ／uレジ管理側なら出せることがある。
  ...
  （過去月ABCは取れると分かった。手作業のエクスポートを頼む前に自動取得を疑う）。
