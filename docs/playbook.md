...
- ⚠ FW ABC は**過去月の商品明細を返さない**（前年/直前は取得不可）。したがって蓄積は
  **今後、毎月 `abc-store-ingest` を全店ぶん回す**運用で前向きに積む。前年比は前年同月の
  ABCが貯まってから出る（それまでは「―」）。`abc-ingest`（全店）はグループ売れ筋(_group)
  のみで、店別の月次には使えない（店別は `abc-store-ingest`）。
- 自動化: `abc-monthly.yml`（毎月6日 05:20 JST）が稼働店を `config/stores.yaml` から動的に
  matrix 化し、前月ABCを1店ずつ（4並行）取り込む。手動は workflow_dispatch（month 指定可）。
