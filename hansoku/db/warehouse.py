...
# pos_sheet を最優先にしてあるのは、**FW側がFW未連動店に 0 を書いている**ため。
# 1766 ぎふや福岡天神は 2026-05〜08 に fw 由来の 0 行があり、既定順位（50）だと
# その 0 が新Uレジの実数（12,640,890）を押しのけていた。
# pos_sheet は `pos` が fw 以外の店にしか行を作らない（pos_sheet.py で弾いている）
# ので、最優先にしてもFW連動店の数字は動かない。
