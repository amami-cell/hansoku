...
class AppDb:
    ...
    # ── 販促の目標（アプリ内入力）────────────────────────────────────────
    def list_promo_targets(self, metric: str = "sales") -> dict[str, int]:
        """施策id → 目標（指標 metric、既定は売上円）の辞書。"""
        return {
            r["campaign_id"]: int(r["target_value"])
            for r in self.query(
                "SELECT campaign_id, target_value FROM promo_targets WHERE metric = %s",
                (metric,),
            )
        }
    ...
    def set_promo_target(
        ...
        """目標を登録／更新する（誰が更新したかを set_by に残す）。指標ごとに1件。"""
        self.execute(
            """
            INSERT INTO promo_targets (campaign_id, metric, target_value, set_by, set_at)
            VALUES (%s, %s, %s, %s, now())
            ON CONFLICT (campaign_id, metric) DO UPDATE SET
                target_value = EXCLUDED.target_value,
                set_by       = EXCLUDED.set_by,
                set_at       = now()
            """,
            (campaign_id, metric, int(target_value), set_by),
        )

    def delete_promo_target(self, campaign_id: str, metric: str = "sales") -> None:
        self.execute(
            "DELETE FROM promo_targets WHERE campaign_id = %s AND metric = %s",
            (campaign_id, metric),
        )
